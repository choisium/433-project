...
class NetworkClient(clientInfo: ClientInfo) {
  ...
  final def requestConnect(): Unit = {
    ...
    val response = blockingStub.connect(new ConnectRequest(clientInfo.masterHost, clientInfo.masterPort))
    id = response.id
  ...
  final def merge(): Unit = {
    ...
    Sorter.getListOfFiles(clientInfo.outputDir).foreach(file => Merger.sortNotTagged(file.getPath))
  }
}
