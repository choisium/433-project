...
import sorting.Sorter.{getListOfFiles, sort, splitSingleInput, whereToPut, writeOrCreateAndWrite}
...
object Merger {
  ...
  // merge shuffled files into ouput-##
  def merge(workerPath: String, subRanges: Seq[(String, String)]):Any = {
    ...
    for (file <- listOfInputFiles) {
      splitSingleInput(file.getPath, workerPath + "/output/shuffle-", " ", Iterator.from(0).zip(subRanges).toMap)
    }
  }
  ...
  def getListOfStageFiles(directoryPath: String, stage:String): List[File] = {
    ...
    if (dir.exists && dir.isDirectory) {
      dir.listFiles.filter(file => file.isFile && file.getPath.take(stage.length).equals(stage)).toList
    } else {
      List[File]()
    }
  }
  ...
  def shuffle(workerPath: String): Any = {
    ...
    for (file <- listOfPartitionedFiles) {
      ...
      val path = Files.move(
        ...
        Paths.get(workerPath + "/../" + workerId + "/shuffle/shuffle-" + workerId + "-1"),
